...
The implementation in the present module was created by the author from the
description of the algorithm published in the paper.  Assuming my
interpretation of the Enslen et al.'s algorithm is correct, this
implementation only differs in a minor optimization where it skips splitting
strings that are less than 2 characters long.
...
The simplest way to use this module in a Python program is to import Spiral
and then call samurai_split() with a string. For example,

    from spiral import samurai_split
    result = samurai_split('someidentifier')

The function samurai_split(...) produces a list of strings as its output.  It
will do its best-guess at how a given identifier should be split using a
built-in table of term frequencies, prefixes, suffixes, and an English
dictionary.  The built-in table of term frequencies came from an analysis of
10,000 randomly selected software projects in GitHub that contained at least
one Python source code file.

Samurai is in fact intended to be used with project-specific tables of term
frequencies.  If you have a set of frequencies for your project, you can
supply them to the function init_samurai(...) once, before making any calls
to samurai_split().  (This is done so that the cost of computations involved
in the frequencies is only done once.)  Use the keyword argument 'local_freq'

    from spiral import samurai_split
    init_samurai(local_freq=myfrequencies)
    result = samurai_split('someidentifier')
...
be passed using the keyword argument 'global_freq' to init_samurai(...):

    init_samurai(global_freq=globalfrequencies, local_freq=projectfrequencies)
...
def score(w):
    f = word_frequency(w)
    # Modified from Samurai: low threshold on frequency values:
